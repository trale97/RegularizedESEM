...
for (i in length(cardinality)){
  a <- IS_CCrESEM(X, R = 3, card = cardinality[i], MaxIter = 200, eps = 10^-6, nstarts = 5)
  IS_genetic[i,] <- c(cardinality[i], a$value, a$vaf)
}
genetic_model <- MULTISTART_CCrESEM(X, R = 3, CARD = cardinality[which.max(IS_genetic[,2])], MaxIter = 200, eps = 10^-6, nstarts = 10)
...
### Analyze using rESEM-l1 (have to try several times with different lambda)
RLSLV_genetic <- MULTISTART_rESEMl1(X, R = 3, MaxIter = 200, eps = 10^-6, nstarts = 10, lambda = 16.85)

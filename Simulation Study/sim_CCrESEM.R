...
Simulation = foreach(i = 1:Ndatasets,
                     ...
                     .combine = rbind)%dopar%{
                       ...
                       for (j in 1:4){
                         a <- IS_CCrESEM(X = data1[,1:(K*3)], R = 3, card = cardmat[j,], MaxIter = 20, eps = 10^-6)
                         b <- IS_CCrESEM(X = data2[,1:(K*3)], R = 3, card = cardmat[j,], MaxIter = 20, eps = 10^-6)
                       ...
                       result1 <- CCrESEM(DATA = data1[,1:(K*3)], R = 3, CARD = rep(avec[selmodelindex1,1],3), MaxIter = 200, eps = 10^-6)
                       result2 <- CCrESEM(DATA = data2[,1:(K*3)], R = 3, CARD = rep(bvec[selmodelindex2,1],3), MaxIter = 200, eps = 10^-6)
                       ...
                       save(output, file = paste0("ORTHOGONAL2/CCrESEM", i, ".RData"))

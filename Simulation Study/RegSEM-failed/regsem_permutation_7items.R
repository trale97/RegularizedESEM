...
model7_list <- list(model7_regsem1, model7_regsem2, model7_regsem3, model7_regsem4, model7_regsem5, model7_regsem6)
...
Simulation_regSEM7 = foreach(i = 701:900,
                             ...
                             .combine = rbind)%dopar%{
                               ...
                               corrate1 <- vector(length = length(model7_list))
                               corrate2 <- vector(length = length(model7_list))
                               spcr_tucongrT1 <- vector(length = length(model7_list))
                               spcr_tucongrT2 <- vector(length = length(model7_list))
                               
                               
                               for (j in 1:length(model7_list)){
                                 fit1 <- sem(model7_list[[j]], data = data1, se = "none")
                                 ...
                                 beta1 <- unname(reg.out1$final_pars[64:66])
                                 beta1_list[[j]] <- beta1
                                 
                                 fit2 <- sem(model7_list[[j]], data = data2, se = "none")
                                 ...
                                 beta2 <- unname(reg.out2$final_pars[64:66])
                                 beta2_list[[j]] <- beta2

...
#########################################################################################
#####                  rESEM-CC function for orthogonal components                   #####
#########################################################################################

##1. rESEM-CC function 
CCrESEM <- function(DATA, R, CARD, MaxIter, eps) {
  J <- dim(DATA)[2]
  I <- dim(DATA)[1]
  convAO <- 0
  iter <- 1
...
#########################################################################################
MULTISTART_CCrESEM<- function(DATA, R, CARD, MaxIter, eps, nstarts){
  if(missing(nstarts)){
    nstarts <- 20
  } 
...
IS_rESEM <- function(X, R, card, MaxIter, eps, nstarts){
